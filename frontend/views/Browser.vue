          ...
          <template slot-scope="props">
            ...
            <b-table-column id="single-actions" width="51">
              <b-dropdown v-if="props.row.type != 'back'" :disabled="checked.length > 0" aria-role="list" position="is-bottom-left">
                ...
                <b-dropdown-item
                  v-if="props.row.type == 'file' && can('download') && isPublicFile(props.row.path)"
                  v-clipboard:copy="getPublicLink(props.row.path)"
                  aria-role="listitem"
                >
                  <b-icon icon="globe" size="is-small" /> {{ lang('Copy public link') }}
                </b-dropdown-item>
              </b-dropdown>
            </b-table-column>
          </template>
